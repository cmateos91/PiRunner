...
class KVStorage {
  ...
  static async addScore(scoreData) {
    try {
      ...
      // Save back to KV
      await kv.set(this.SCORES_KEY, existingScores);
  ...
  static async updateLeaderboardCache(scores) {
    try {
      ...
      await kv.set(this.LEADERBOARD_KEY, leaderboards);
  ...
  static async clearAll() {
    try {
      ...
      await kv.del(this.LEADERBOARD_KEY);
      console.log('All KV data cleared');
    } catch (error) {
